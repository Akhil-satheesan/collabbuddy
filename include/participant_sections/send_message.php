<?php
// FILE: include/participant_sections/send_message.php (Participant AJAX)

require_once __DIR__ . '/../../include/config.php';
if (session_status() === PHP_SESSION_NONE) session_start();
header('Content-Type: application/json');

if (!isset($_SESSION['user_id']) || $_SESSION['role'] !== 'participant' || !isset($_POST['group_id']) || !isset($_POST['message'])) {
...
$auth_sql = "SELECT COUNT(*) FROM group_chat_members gcm WHERE gcm.group_id = ? AND gcm.user_id = ?";
$auth_stmt = $conn->prepare($auth_sql);
$auth_stmt->bind_param("ii", $group_id, $participant_id);
$auth_stmt->execute();
...
if ($stmt) {
    $stmt->bind_param("iis", $group_id, $participant_id, $message_text);
    if ($stmt->execute()) {
